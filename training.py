...
class LanguageModel(object):
    ...
    '''
    def __init__(self, config, options, is_training, contexts, num_ctx, dim_ctx):
        self.config = config
        ...
        if self.is_training:
            ...
            self.token_ids = tf.placeholder(
                dtype = tf.int32,
                shape = [config.batch_size, config.max_caption_length], name='token_ids') # [32,20]
            self.masks = tf.placeholder(
                dtype = tf.float32,
                shape = [config.batch_size, config.max_caption_length], name='masks')
        else:
            self.contexts = tf.placeholder(
                dtype = tf.float32,
                shape = [config.batch_size, self.num_ctx, self.dim_ctx], name='contexts')
    ...
    def _build_word_embeddings(self):
        ...
        if self.bidirectional:
            self.token_ids_reverse = tf.placeholder(DTYPE_INT,
                               shape=(self.config.batch_size, self.config.max_caption_length),
                               name='token_ids_reverse')
            self.masks_reverse = tf.placeholder(
                dtype = tf.float32,
                shape = [self.config.batch_size, self.config.max_caption_length],name='mask_reverse')
            with tf.device("/cpu:0"):
    ...
    def _build_word_char_embeddings(self):
        ...
        self.tokens_characters = tf.placeholder(DTYPE_INT,
                                   shape=(self.config.batch_size, self.config.max_caption_length, max_chars),
    ...
    def _build(self):
        ...
        for lstm_num, lstm_input in enumerate(lstm_inputs):
            ...
            word_embed = tf.transpose(lstm_input,[1,0,2])
            with tf.variable_scope("initialize",reuse=tf.AUTO_REUSE):
                context_mean = tf.reduce_mean(self.contexts, axis=1)
                initial_memory, initial_output = self.initialize(context_mean)
                initial_state = initial_memory, initial_output
            if self.is_training:
                last_output = initial_output
                last_memory = initial_memory
                last_word = tf.zeros([self.config.batch_size], tf.int32)
            else:
                num_steps = 1
                    ...
                    with tf.variable_scope('RNN_%s' % lstm_num):
                        for idx in range(self.config.max_caption_length):
                            ...
                            with tf.variable_scope("attend",reuse=tf.AUTO_REUSE):
                                alpha = self.attend(self.contexts, last_output)
                                context = tf.reduce_sum(self.contexts * tf.expand_dims(alpha, 2),
                                                        axis=1)  # [32,49,2048]-> [32,2048]
                                if self.is_training:
                                    if lstm_num == 0 :
                                        ...
                                        tiled_masks = tf.tile(tf.expand_dims(self.masks_reverse[:, idx], 1),
                                                              [1, self.num_ctx])
                            ...
                            with tf.variable_scope("lstm"):
                                current_input = tf.concat([context, word_embed[idx]], 1)
                                output, state = lstm_cell(current_input, last_state)
                                memory, _ = state
                                _lstm_output_unpacked.append(output)
                                self.final_lstm_state.append(state)
                            ...
                            sentences = self.token_ids
                            # Compute the loss for this step, if necessary

                            if self.is_training:
                                if lstm_num == 0:
                                    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(
                                        labels=self.token_ids[:, idx],
                                        logits=logits)
                                    masked_cross_entropy = cross_entropy * self.masks[:, idx]
                                    cross_entropies.append(masked_cross_entropy)

                                    ground_truth = tf.cast(self.token_ids[:, idx], tf.int64)
                                    ...
                                    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(
                                        labels=self.token_ids_reverse[:, idx],
                                        logits=logits)
                                    masked_cross_entropy = cross_entropy * self.masks_reverse[:, idx]
                                    cross_entropies.append(masked_cross_entropy)

                                    ground_truth = tf.cast(self.token_ids_reverse[:, idx], tf.int64)
                                    prediction_correct = tf.where(
                                        tf.equal(prediction, ground_truth),
                                        tf.cast(self.masks_reverse[:, idx], tf.float32),
                                        tf.cast(tf.zeros_like(prediction), tf.float32))
                                    predictions_correct.append(prediction_correct)
                                last_output = output
                                last_memory = memory
        ...
        if self.is_training:
            ...
            alphas = tf.reshape(alphas, [self.config.batch_size, self.num_ctx, -1])
            attentions = tf.reduce_sum(alphas, axis = 2)
            diffs = tf.ones_like(attentions) - attentions
            attention_loss = self.config.attention_loss_factor \
                             * tf.nn.l2_loss(diffs) \
                             / (self.config.batch_size * self.num_ctx)
...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    if restart_ckpt_file is None:
        with open(os.path.join(tf_save_dir, 'model_option.json'), 'w') as fout:
...
def load_options_latest_checkpoint(tf_save_dir):
    options_file = os.path.join(tf_save_dir, 'model_option.json')
