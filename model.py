...
#from bilmatt import BiLSTMAttention
from bilm.training import train, load_options_latest_checkpoint, load_vocab, LanguageModel
from bilm.data import Vocabulary, UnicodeCharsVocabulary, InvalidNumberOfCharacters
...
class CaptionGenerator(BaseModel):
    ...
    #             dtype = tf.int32,
    #             shape = [config.batch_size, config.max_caption_length]) # [32,20]
    ...
    #             trainable = self.is_train)  #[5000,512]
    #
    #     # Setup the LSTM
    ...
    # def attention(self): #self-attention
    #     x = self.conv_feats
    #     ch = self.dim_ctx
    #     self.sn = True # spectral norm
    ...
    #     x = gamma * o + x
    #
    #     self.context = tf.reshape(x,[self.config.batch_size, self.num_ctx, 2048])

    def build_bilm(self):
        config = self.config
        #cnn = BiLSTMAttention(config ,self.context, self.num_ctx) #接收CNN输出的特征
        ...
        bilstm = LanguageModel(config, options, self.is_train, self.conv_feats, self.num_ctx, self.dim_ctx)
        self.contexts = bilstm.contexts
        self.init_lstm_state = bilstm.init_lstm_state
        self.final_lstm_state = bilstm.final_lstm_state

        if self.is_train:
            self.total_loss = bilstm.total_loss
            self.token_ids = bilstm.token_ids
            self.token_ids_reverse = bilstm.token_ids_reverse
            self.masks = bilstm.masks
            self.masks_reverse = bilstm.masks_reverse
            self.embedding = bilstm.embedding
            self.accuracy = bilstm.accuracy
            self.cross_entropy_loss = bilstm.cross_entropy_loss
    ...
    def build_summary(self):
        ...
        with tf.name_scope("metrics"):
            ...
            tf.summary.scalar("cross_entropy_loss", self.total_loss)
