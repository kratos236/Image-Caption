...
def prepare_train_data(config):
    ...
    if not os.path.exists(config.vocabulary_file) or not os.path.exists(config.temp_annotation_file):
    ...
    dataset = DataSet(image_ids,
                      ...
                      True)
    print("Dataset built.")
    # dump_token_embeddings(config.vocabFile, config.dump_option, config.weightFile, config.tokenEmbeddingFile) #只需dump第一次即可
    return dataset
